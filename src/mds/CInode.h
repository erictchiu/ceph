...
public:
  ...
  InodeStore(const InodeStore *o=NULL, bool no_alloc=false)
  {
    if (o) {
      inode = o->inode;
      xattrs = o->xattrs;
...
public:
  ...
  // ---------------------------
  CInode(MDCache *c, bool auth=true, snapid_t f=2, snapid_t l=CEPH_NOSNAP, InodeStore *o=NULL) : 
    InodeStore(o),
    mdcache(c),
...
public:
  ...
  void finish_scatter_gather_update(int type, inode_t *pi);
